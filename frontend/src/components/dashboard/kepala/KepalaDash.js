      ...
      {!showReview ? (
        <>
          ...
          <div className="flex shrink-0 flex-row flex-wrap justify-center gap-5 md:justify-start">
            ...
            <CardReport
              label="Ongoing"
              value={mockReports2.filter((r) => r.status === 1 && !r.tertolak).length}
          ...
          <div className="flex flex-1 flex-col overflow-hidden">
            ...
            <div className="mt-2 flex w-full flex-1 flex-col gap-2 overflow-hidden rounded-2xl border border-[#C4C4C4]/20 bg-[#2B2E4D]/20 p-2">
              <div className="relative h-[70vh] overflow-y-auto pr-2 md:h-full">
                {mockReports2
                  .filter((report) => report.status === 1 && !report.tertolak)
                  .map((report) => (
                    <div
                      key={report.idSurat}
        ...
        <div className="relative">
          ...
          <KepalaReview
            reportData={{
              reportTitle: selectedReport?.idSurat || "",
              employeeName: selectedReport?.employeeName || "",
              employeeId: selectedReport?.uid || "",
              department: selectedReport?.departemen || "",
              severity: selectedReport?.severity || "Minor",
              incidentDate: selectedReport?.tanggal || "",
              description: selectedReport?.description || "No description available",
            }}
